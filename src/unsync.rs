...
/// A basic safe single-threaded [red-black tree](https://en.wikipedia.org/wiki/Red%E2%80%93black_tree) implementation.
/// # Example
/// ```
/// use red_black_tree::unsync::RedBlackTree;
///
...
impl<T: Clone + Ord> RedBlackTree<T> {
    ...
    pub fn remove(&mut self, val: T) -> bool {
        ...
        while !removal_node.as_ref().unwrap().borrow().is_leaf() {
            ...
            if removal_node.as_ref().unwrap().borrow().right.is_some() {
                let mut right_subtree_min = removal_node.as_ref().unwrap().borrow().right.clone();

                while right_subtree_min.as_ref().unwrap().borrow().left.is_some() {
                    right_subtree_min = right_subtree_min.take().unwrap().borrow().left.clone();
                }

                removal_node.as_ref().unwrap().borrow_mut().val = right_subtree_min.as_ref().unwrap().borrow().val.clone();
                removal_node = right_subtree_min;
            } else {
                let mut left_subtree_max = removal_node.as_ref().unwrap().borrow().left.clone();

                while left_subtree_max.as_ref().unwrap().borrow().right.is_some() {
                    left_subtree_max = left_subtree_max.take().unwrap().borrow().right.clone();
                }

                removal_node.as_ref().unwrap().borrow_mut().val = left_subtree_max.as_ref().unwrap().borrow().val.clone();
        ...
        if removal_node.as_ref().unwrap().borrow().color == Color::Black {
    ...
    fn resolve_double_black(&mut self, db_node: Link<T>, db_direction: Option<Direction>) {
        ...
        let sibling = match db_direction {
            Direction::Left => db_node.as_ref().unwrap().borrow().parent.as_ref().and_then(|weak| weak.upgrade()).as_ref().unwrap().borrow().right.clone(),
            Direction::Right => db_node.as_ref().unwrap().borrow().parent.as_ref().and_then(|weak| weak.upgrade()).as_ref().unwrap().borrow().left.clone(),
        };
            ...
            Color::Black => {
                ...
                match (sibling_far_child_color, sibling_near_child_color) {
                    ...
                    (Color::Red, _) => {
                        ...
                        parent.as_ref().unwrap().borrow_mut().color = Color::Black;
                        sibling.as_ref().unwrap().borrow_mut().color = parent_color;
                        // grab sibling's far child
                        let sibling_far_child = match db_direction {
                            Direction::Left => sibling.as_ref().unwrap().borrow().right.clone(),
                            Direction::Right => sibling.as_ref().unwrap().borrow().left.clone(),
                        };
                        // rotate at parent node towards db_node
                        self.rotate_subtree(parent, db_direction);
                        // color sibling_far_child black
                        sibling_far_child.as_ref().unwrap().borrow_mut().color = Color::Black;
                    }
                }
            },
            Color::Red => {
                ...
                parent.as_ref().unwrap().borrow_mut().color = Color::Red;
                sibling.as_ref().unwrap().borrow_mut().color = parent_color;
                // rotate at parent node towards db_node
                self.rotate_subtree(parent, db_direction);
    ...
    fn rebalance_at_point(&mut self, node: Link<T>) {
        ...
        if parent.as_ref().unwrap().borrow().color == Color::Black {
            // nothing to do here
            return;
        }
    ...
    fn rotate_subtree(&mut self, subtree_root: Link<T>, direction: Direction) {
        ...
        let pivot = match direction {
            Direction::Left => {
                ...
                subtree_root.as_ref().unwrap().borrow_mut().right = pivot.as_ref().unwrap().borrow().left.clone();
            ...
            Direction::Right => {
                ...
                subtree_root.as_ref().unwrap().borrow_mut().left = pivot.as_ref().unwrap().borrow().right.clone();
    ...
    fn inorder_traversal_recursive(node: Link<T>) -> Vec<Link<T>> {
        ...
        vec.append(&mut Self::inorder_traversal_recursive(node.as_ref().unwrap().borrow().left.clone()));
        vec.push(node.clone());
        vec.append(&mut Self::inorder_traversal_recursive(node.as_ref().unwrap().borrow().right.clone()));
        vec
    }
}
...
impl<T: Clone + Ord> From<RedBlackTree<T>> for Vec<T> {
    fn from(value: RedBlackTree<T>) -> Self {
        ...
        value.inorder_traversal().into_iter().map(|node| node.as_ref().unwrap().borrow().val.clone()).collect()
    }
}
